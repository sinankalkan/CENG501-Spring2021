...
def train(dataset, net, batch_size, learning_rate, optimizer, lr_decay_step,
    ...
    temp_params = add_params
    temp_params['devkit_path'] = 'VOCdevkit_foggy'
    tdataset = dataset


    dataset, ds_name = dataset_factory.get_dataset(dataset, add_params)
    loader = DataLoader(dataset, batch_size=int(cfg.TRAIN.BATCH_SIZE/2),
                        shuffle=True, collate_fn=collate_train)


    tdataset, tds_name = dataset_factory.get_dataset(tdataset, temp_params)
    tloader = DataLoader(tdataset, batch_size=int(cfg.TRAIN.BATCH_SIZE/2),
                        shuffle=True, collate_fn=collate_train)
    ...
    for current_epoch in range(start_epoch, total_epoch + 1):
        loss_temp = 0
        start = time.time()
        target_iterator = iter(tloader)
        for step, data in enumerate(loader):
            try:
              data2 = next(target_iterator)
            ...
            image_data_t = data2[0].to(device)
            image_info_t = data2[1].to(device)
            gt_boxes_t = data2[2].to(device)


            cls_score, bbox_pred, rpn_loss_cls, \
            rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, \
            D1_o, D2_o, D3_o, D4_o, caia_o = faster_rcnn(image_data, image_info, gt_boxes, target = False)

            d1loss_s = 0.5 * torch.mean(D1_o ** 2)
            d2loss_s = 0.5 * torch.mean(D2_o ** 2)
            d3loss_s = 0.5 * torch.mean(D3_o ** 2)
            d4loss_s = 0.5 * torch.mean(D4_o ** 2)

            loss = rpn_loss_cls.mean() + rpn_loss_bbox.mean() \
                + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean()
            loss_temp += loss.item()

            if current_epoch > int(total_epoch/3):
              caia_loss_s = torch.sum(torch.square(caia_o) * cls_score)/256
              loss += caia_loss_s
            cls_score, bbox_pred, rpn_loss_cls, \
            rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, \
            D1_o, D2_o, D3_o, D4_o, caia_o = faster_rcnn(image_data_t, image_info_t, gt_boxes_t, target = True)

            if current_epoch > int(total_epoch/3):
              caia_loss_t = torch.sum(torch.square(1-caia_o) * cls_score)/256
              loss += caia_loss_t
            d1loss_t = 0.5 * torch.mean((1 - D1_o) ** 2)
            d2loss_t = 0.5 * torch.mean((1 - D2_o) ** 2)
            d3loss_t = 0.5 * torch.mean((1 - D3_o) ** 2)
            d4loss_t = 0.5 * torch.mean((1 - D4_o) ** 2)

            dlosstot = d1loss_s + d2loss_s + d3loss_s + d4loss_s + d1loss_t + d2loss_t + d3loss_t + d4loss_t
            loss += dlosstot
