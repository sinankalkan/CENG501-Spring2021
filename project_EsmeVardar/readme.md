...
In original setup authors use SIM10K to Cityscapes and Cityscapes to Foggy Cityscapes dataset. Because of lack of disk space in Colab at this part we could only implement Cityscapes to Foggy Cityscapes to some extent. In these datasets we have only taken berlin, leverkusen, aachen, hamburg, hanover, stuttgart, zurich and frankfurt as the datasets from the Cityscapes datasets. Learning rate is kept the same as 0.001 as in the paper but the iteration count couldn't be kept the same, authors provided that they trained the network for 70K iterations which we couldn't train for that long as Colab limits gpu usage so we have presented the results for 6K iteration training which is significantly lower than the author's setup. All the other model parameters and settings were applied the same. VGG-16 is used with a baseline faster rcnn model as mentioned in the article. ROI-Align is also used in our implementation for similarity to the paper.
