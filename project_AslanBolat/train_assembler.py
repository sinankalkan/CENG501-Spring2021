...
def train(model, epochs, dataloader, optimizer, 
    ...
    for epoch in range(epochs):
        ...
        if epoch % 9 == 0 or epoch +1 == epochs:
            torch.save(model.state_dict(), "./models/trained_assembler/assembler_final_logcosh_ancor2{}.pt".format(epoch))
...
if __name__=="__main__":
    ...
    dataset = AssemblerDataset(file_path, processed_path, "03001627", 2, 
    ...
    train_loss_history = train(model, 200, dataloader, optimizer)

    plt.plot(train_loss_history)
    plt.xlabel('Iteration number')
    plt.ylabel('Loss value')
    plt.savefig("plots/assembler_loss_logcosh_ancor2.png")
    plt.clf()