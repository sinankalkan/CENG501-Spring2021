...
from lib.utils import AverageMeter, SobelOperator
import lib.metrics as metrics


class MosHTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(tbar):
            ...
            pert_mask = data['pert_mask'].to(self.device)
            input = torch.cat((image, pert_mask), dim=1)

            self.optimizer.zero_grad()

            # LOSS & OPTIMIZE
            loss = 0
            # predictions = [s8_0_out, s8_1_out, s8_2_out, s8_3_out, s4_2_out, s4_3_out, s2_3_out]
            predictions, uc_maps = self.model(input)
            for cri_idx, pred in enumerate(predictions):
                loss += self.criterion_bce(pred, label) * self.cri_weights[cri_idx][0]
                loss += self.criterion_l1(pred, label) * self.cri_weights[cri_idx][1]
                loss += self.criterion_l2(pred, label) * self.cri_weights[cri_idx][2]

            gt_sobel = self.sobel(label)
            pred_sobel = self.sobel(predictions[-1])
            loss += self.criterion_l1(gt_sobel, pred_sobel)
            ...
            if batch_idx % self.log_per_batch == 0:
                ...
                if train_vis_count < self.vis_count and self.train_vis:
                    ...
                    self._visualize_batch(image, label, pert_mask, predictions, batch_idx, vis_save_dir, predef='mask')
                    self._visualize_batch(image, label, pert_mask, uc_maps, batch_idx, vis_save_dir, predef='ucmap')
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(tbar):
                ...
                loss = 0
                predictions, _ = self.model(input)
                ...
                if val_vis_count < self.vis_count and self.val_vis:
                    ...
                    self._visualize_batch(image, label, pert_mask, predictions, batch_idx, vis_save_dir)
    ...
    def _visualize_batch(self, img, label, pert_mask, outputs, step, vis_save_dir, vis_shape=(128, 128), predef=''):
        ...
        if pert_mask.max() == 255:
            pert_mask = pert_mask / 255.

        label = F.interpolate(label, size=vis_shape, mode='bilinear', align_corners=True)
        pert_mask = F.interpolate(pert_mask, size=vis_shape, mode='bilinear', align_corners=True)
        vis_list = [
                       img.cpu().detach(),
                       label.cpu().detach().repeat(1, 3, 1, 1),
                       pert_mask.cpu().detach().repeat(1, 3, 1, 1)
                   ] + outputs
